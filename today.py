...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    # Retry on temporary GitHub errors (502/503/504) before giving up.
    for attempt in range(5):
        request = requests.post('https://api.github.com/graphql', json={'query': query, 'variables':variables}, headers=HEADERS)
        if request.status_code == 200 or request.status_code not in (502, 503, 504):
            break
        time.sleep(2 ** attempt)   # 1s, 2s, 4s, 8s, 16s
    if request.status_code == 200:
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    root = tree.getroot()
    # Stats are a single column; each value is right-justified to the card edge.
    # Field width = 62 - len(label) keeps the value ending at the same right column.
    justify_format(root, 'repo_data', repo_data, 57)        # label "Repos"
    justify_format(root, 'contrib_data', contrib_data, 51)  # label "Contributed"
    justify_format(root, 'star_data', star_data, 57)        # label "Stars"
    justify_format(root, 'commit_data', commit_data, 55)    # label "Commits"
    justify_format(root, 'follower_data', follower_data, 53)# label "Followers"
...
if __name__ == '__main__':
    ...
    Based on the profile card by Andrew Grant (Andrew6rant), 2022-2025.
    """
    print('Calculation times:')
    # define global variable for owner ID and read the account creation date
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    # "Uptime" is measured from the day this GitHub account was created
    account_birth = datetime.datetime.strptime(acc_date, '%Y-%m-%dT%H:%M:%SZ')
    age_data, age_time = perf_counter(daily_readme, account_birth)
